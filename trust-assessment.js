...
/**
 * Track the request of a dataset from one user to another
 * @param {org.example.mynetwork.Request} request - the request to be processed
 * @transaction
 */
async function tradeCommodity(request) {
    request.dataset.user = request.newUser;
    let assetRegistry = await getAssetRegistry('org.example.mynetwork.Dataset');
    await assetRegistry.update(request.dataset);
}